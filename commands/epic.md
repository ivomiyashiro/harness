...
Before any other action, run `node scripts/harness-input.js epic "$ARGUMENTS"`. Stop on failure.
Use only the returned `feature` value as `<epic>` below; never use `$ARGUMENTS` in a path, ref, prompt, or command.
